...
disp('<strong>Doing STEP 2 - Organize micrographs and output list file.</strong>');
...
for i=1:length(stacks)
    ...
    B=split(listName{1,c},'_',2); % 2 is the dimension of the array so all tables are split equally
    SizeB=size(B);
...
disp('<strong>Doing STEP 3 - Creating stack using newstack.</strong>');

%Input files

ts_list=dir('list_ts_*.txt');
tmpA=struct2table(ts_list);
ts_filename=tmpA.name;
ts_number=str2double(extractBetween(ts_filename,'list_ts_','.txt'));

%Loop to create stacks using file list created in STEP3 
    for i=1:length(ts_list)
        disp(['Creating stack for tiltseries number ' num2str(ts_number(i,1)) ' !!!' ])
        system([newstack ' -filei ' char(ts_filename(i,1)) ' -output ts_' num2str(ts_number(i,1)) '.mrc']);
    end
